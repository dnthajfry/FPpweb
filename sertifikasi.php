...
// Ambil data user dari database berdasarkan user ID (atau session)
$userId = 1; // Contoh user ID, sesuaikan dengan mekanisme login Anda
$query = $pdo->prepare("SELECT * FROM users WHERE id = :id");
$query->execute(['id' => $userId]);
$user = $query->fetch(PDO::FETCH_ASSOC);

// Ambil data sertifikasi dari database
$queryCert = $pdo->prepare("SELECT * FROM certifications WHERE user_id = :id");
$queryCert->execute(['id' => $userId]);
$certification = $queryCert->fetch(PDO::FETCH_ASSOC);
